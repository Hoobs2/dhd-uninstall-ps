...
$msiPrograms = [ordered]@{
    ...
    "Program14" = "/X{4B9E6EB0-0EED-4E74-9479-F982C3254F71} /q /NORESTART /Log C:\itslog\Intune\Intune_DHDUninstall_SQL2012SERVERBROWSER.txt"
    ...
    "Program16" = "/X{B40EE88B-400A-4266-A17B-E3DE64E94431} /q /NORESTART /Log C:\itslog\Intune\Intune_DHDUninstall_SQL2008SETUP.txt"
...
function UninstallPrograms {
    ...
    foreach ($program in $programs.GetEnumerator()) {
        ...
        $path = $uninstallInfo.Path
        $args = $uninstallInfo.UninstallArgs

        $uninstallExecutable = Join-Path -Path $path -ChildPath "uninstall.exe"

        if (Test-Path $uninstallExecutable) {
            Start-Process $uninstallExecutable -ArgumentList $args -Wait -WindowStyle Hidden | Out-Null
        }
    }
}
...
function CheckUninstalledPrograms {
    ...
    foreach ($program in $registryKeys.Keys) {
        ...
        if ($keyExists -and $programs.ContainsKey($program)) {
            UninstallPrograms -programs $programs[$program]
        }
    }
}
...
TestITSLog
UninstallPrograms -programs $msiPrograms
UninstallPrograms -programs $exePrograms
CheckUninstalledPrograms -programs $exePrograms -registryKeys $registryKeys
